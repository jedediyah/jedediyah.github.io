...
var new_game_recently_created = false; 
function new_game(game_type) {

	if (game_mode==2 && !game_over) { return false; }

	if (new_game_recently_created) { return false; }
	new_game_recently_created = true;
	setTimeout( function() { new_game_recently_created=false; }, 750);
    ...
    if(game_mode==2) { setTimeout( function(){machine_move()},350); } // Start game if mvm
    //if(game_mode==2) { machine_move(); }
}
...
function machine_move() {
	...
	if (legal_moves.length==1) {
        setTimeout( function() {make_move(legal_moves[0])},350 );		
	...
	if (known_next_states==null || known_next_states.length<1) {
        ...
        //make_move_from_state(best_known_move); 
